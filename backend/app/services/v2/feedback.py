...
def process_feedback(
    ...
    # --- 2. 피드백 텍스트 벡터화 ---
    try:
        feedback_vector = sbert_model.encode(req.text_content)
    except Exception as e:
        raise RuntimeError(f"SBERT encoding failed: {e}")

    # --- 3. 벡터 미세 조정 (핵심 로직) ---
    current_vector = np.array(category.embedding)
    
    if req.feedback_type == "reinforce":
        # "reinforce": 대표 벡터를 피드백 벡터 쪽으로 '가깝게' 이동
        # (1 - 0.05) * 현재벡터 + 0.05 * 피드백벡터
        new_vector = (1 - LEARNING_RATE) * current_vector + LEARNING_RATE * feedback_vector
    ...
    new_log = FeedbackLog(
        ...
        text_embedding=feedback_vector.tolist(), # 피드백 텍스트의 벡터도 저장
    ...
    category.embedding = new_vector.tolist() # NumPy 배열을 list로 변환하여 저장
