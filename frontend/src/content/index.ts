// 콘텐츠 스크립트에서 직접 사용할 최소 타입과 유틸을 정의 (모듈 임포트 없이 동작하도록 작성)
...
function subscribeConfigChanges(
  ...
  chrome.storage.onChanged.addListener((changes, areaName) => {
    ...
    const updated = changes[STORAGE_KEY]?.newValue as StoredConfig | undefined;
    if (updated) {
      callback(updated);
    }
  });
...
function getConfigKey(config: StoredConfig): string {
  return `${config.apiBaseUrl || DEFAULT_API_BASE_URL}|${config.token ?? ""}`;
}

function createFallbackResult(text: string): FilterResult {
  return {
    text,
    should_filter: false,
    matched_categories: [],
  ...
  try {
    ...
    response.results.forEach((result, index) => {
      ...
      console.log("[WebPurifier] 필터 응답", {
        ...
        matched: normalized.matched_categories.map(
          (item: MatchedCategoryInfo) => item.name
        ),
      });
...
const processedText = new WeakMap<Text, string>();
const blurredElements = new Set<Element>();
const pendingElements = new Set<Element>();
let isProcessing = false;
// 노드별 디바운스 타이머, 진행 중 요청 수
const debounceTimers = new WeakMap<Text, number>();
...
function dequeueNode(): Text | undefined {
...
let selectionStatus: HTMLSpanElement | null = null;
let selectionPreview: HTMLSpanElement | null = null;
let selectionSendButton: HTMLButtonElement | null = null;
let selectionCurrentText: string | null = null;
...
chrome.runtime.onMessage.addListener((message, _sender, _sendResponse) => {
  const typedMessage = message as RuntimeMessage | undefined;
...
function applyConfig(config: StoredConfig) {
...
function scanNode(node: Node) {
  ...
  while ((next = walker.nextNode())) {
    enqueueNode(next as Text);
  }
}

function enqueueNode(node: Text) {
...
function queueNode(node: Text): void {
  const flagged = node as Text & { __webpurifierQueued?: boolean };
...
function removeBlur(element: Element | null) {
  if (!element) {
    return;
  }
...
function clearPending(element: Element | null): void {
  if (!element) {
    return;
  }
...
function ensureSelectionControls(): void {
  if (!selectionButton) {
...
function handleSelectionOutsideClick(event: MouseEvent): void {
  if (!selectionPanel || selectionPanel.style.display === "none") {
    return;
  }
...
function updateSelectionButtonPosition(): void {
...
function openSelectionPanel(): void {
  if (
    !selectionPanel ||
    !selectionSelect ||
    !selectionStatus ||
...
function populateSelectionCategories(categories: CategoryInfo[]): void {
...
function positionSelectionPanel(): void {
  if (!selectionPanel) {
    return;
  }
...
function hideSelectionControls(): void {
  if (selectionButton) {
    selectionButton.style.display = "none";
  }
  closeSelectionPanel();
...
function scheduleSelectionStatusClear(): void {
  if (selectionStatusTimer) {
    window.clearTimeout(selectionStatusTimer);
  }
...
async function submitSelectionFeedback(): Promise<void> {
...
function isSelectionWithinBlurred(selection: Selection): boolean {
  const anchorNode = selection.anchorNode;
  const focusNode = selection.focusNode;
  return isNodeWithinBlurred(anchorNode) || isNodeWithinBlurred(focusNode);
}

function isNodeWithinBlurred(node: Node | null): boolean {
...
function attachFeedbackControls(
  element: Element,
  text: string,
  categories: MatchedCategoryInfo[]
): void {
...
function updateFeedbackSelect(context: FeedbackContext): void {
  ...
  const hasPrevious = context.categories.some(
    (cat) => String(cat.id) === previousValue
  );
...
function teardownFeedbackListeners(): void {
  if (!feedbackListenersAttached) {
    return;
  }
...
function detachFeedback(element: Element): void {
  const context = feedbackContextMap.get(element);
  if (!context) {
    return;
  }
...
function setFeedbackBusy(context: FeedbackContext, active: boolean): void {
...
function scheduleStatusClear(context: FeedbackContext): void {
  if (context.statusTimer) {
    window.clearTimeout(context.statusTimer);
  }
...
function getSelectedCategoryId(context: FeedbackContext): number | null {
  if (context.categories.length === 0) {
    return null;
  }
